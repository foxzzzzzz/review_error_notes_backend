...
class SheetItem(Base):
    ...
    wrong_question_id = Column(UUID(as_uuid=True), ForeignKey("wrong_questions.id"), nullable=False)
    question_type = Column(ENUM("original", "derived", name="item_type_enum"), nullable=False)
    derived_from = Column(UUID(as_uuid=True), ForeignKey("wrong_questions.id"), nullable=True)
