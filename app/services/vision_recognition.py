...
7. 正例：印刷词语“课文”要求写拼音，即使只圈出 kè，也要输出 prompt_text=课文、raw_text=学生写出的完整“kè wén”、answer=正确完整“kè wén”、question_type=write_pinyin。印刷词语“算式”同理，应保留完整“算式”及其两个拼音，不能只保留 suàn。
8. 正例：印刷拼音“hé zuò”要求写词语，学生写成“合做”且只圈出“做”时，要输出 prompt_text=hé zuò、raw_text=合做、answer=合作、question_type=write_word。以上情况不得只输出 kè、suàn 或 做。
9. 边界示例：一行包含“识字、组词、计算、图形、合唱”，只有“计算”的拼音被红圈标错时，raw_text 只包含“计算”及其学生对整个词语写出的拼音，不得包含“识字、组词、图形、合唱”。
...
14. instruction 必须填写该独立作答单元的原始练习要求，例如“看词语写拼音”“看拼音写词语”；prompt_text 必须填写重新出卷时展示的干净提示材料，例如“计算”或“hé zuò”。二者不得包含学生作答、正确答案或老师批改笔迹。
...
要求：
1. 每个 bbox 必须覆盖该题的印刷提示、学生答案和相关红色批改标记。
2. 不得包含未标记的相邻兄弟小题，也不得把一个题目的 bbox 配给另一个题目。
3. bbox 使用归一化角点格式 [left, top, right, bottom]。
4. 对下面每个 index 恰好返回一次，不得缺失、重复或新增 index。
5. confidence 表示定位到完整独立作答区域的置信度，范围 0 到 1。
...
返回格式：
{"items":[{"index":0,"bbox":[0.0,0.0,1.0,1.0],"confidence":0.95}]}
"""
