...
11. instruction 必须填写该独立作答单元的原始练习要求，例如“看词语写拼音”“看拼音写词语”；prompt_text 必须填写重新出卷时展示的干净提示材料，例如“计算”或“hé zuò”。二者不得包含学生作答、正确答案或老师批改笔迹。
12. question_type 只能是 write_pinyin、write_word、fill_blank、calculation、other 之一。normalized_text 可填写该单元的规范写法；answer 可填写正确答案。无法确认时保留原样并写入 uncertain_segments。
13. confidence 是对 raw_text、红色标记关联和分组正确性的综合置信度，范围 0 到 1。
14. 只输出一个 JSON 对象，不要解释，不要 Markdown。
